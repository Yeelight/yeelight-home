...
func TestInvokeDeviceUnbindRequiresExplicitConfirmationBeforeExecuting(t *testing.T) {
	deviceVisible := true
	var gotCalls []string
...
func TestInvokeDeviceUnbindResolvesNaturalDeviceNameBeforeConfirmation(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(writer http.ResponseWriter, request *http.Request) {
		writer.Header().Set("Content-Type", "application/json")
		switch request.URL.Path {
		case "/apis/iot/v2/thing/manage/house/200171/device/r/info/1/100":
	...
	defer server.Close()
	t.Setenv("YEELIGHT_API_BASE_URL", server.URL+"/apis/iot")
	app := newInvokeTestApp(t, "Bearer token-unbind-name-secret", "client-unbind-name-1", "200171")

	input := `{"contractVersion":"1.0","requestId":"req-device-unbind-name","locale":"zh-CN","utterance":"解绑主灯","intent":"device.unbind","parameters":{"houseId":"200171","deviceName":"主灯","clearMac":true}}`
	var stdout bytes.Buffer
	var stderr bytes.Buffer
	code := app.run([]string{"invoke", "--stdin"}, strings.NewReader(input), &stdout, &stderr)
	if code != exitOK {
	...
	preview := clarification["preview"].(map[string]any)
	payload := preview["payloadPreview"].(map[string]any)
	if payload["deviceId"] != "50018330" {
		t.Fatalf("payload = %#v", payload)
	}
}

func TestInvokeDeviceUnbindExecutesAfterExplicitConfirmation(t *testing.T) {
	deviceVisible := true
	var gotCalls []string
	server := httptest.NewServer(http.HandlerFunc(func(writer http.ResponseWriter, request *http.Request) {
		gotCalls = append(gotCalls, request.Method+" "+request.URL.Path)
	...
	input := `{"contractVersion":"1.0","requestId":"req-device-unbind-confirmed","locale":"zh-CN","utterance":"确认解绑主灯","intent":"device.unbind","parameters":{"houseId":"200171","deviceId":"50018330","clearMac":true,"unbindRelDevices":true,"confirmed":true}}`
	var stdout bytes.Buffer
	var stderr bytes.Buffer
	code := app.run([]string{"invoke", "--stdin"}, strings.NewReader(input), &stdout, &stderr)
	if code != exitOK {
