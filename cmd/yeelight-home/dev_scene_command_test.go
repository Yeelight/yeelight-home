...
func TestDevSeedSceneRequiresAllowWriteDev(t *testing.T) {
	var stdout bytes.Buffer
	var stderr bytes.Buffer

	code := newTestApp(t).run([]string{"dev", "seed-scene", "--json", "--region", "dev", "--house-id", "200171", "--device-id", "50018330", "--name", "Codex Dev Test Scene"}, strings.NewReader(""), &stdout, &stderr)
...
func TestDevSeedSceneRequiresHouseAndDeviceID(t *testing.T) {
	var stdout bytes.Buffer
	var stderr bytes.Buffer
	app := newTestApp(t)
	...
	code := app.run([]string{"dev", "seed-scene", "--json", "--region", "dev", "--name", "Codex Dev Test Scene", "--allow-write-dev"}, strings.NewReader(""), &stdout, &stderr)
	if code != exitInvalidInput {
		t.Fatalf("exit code = %d, stdout = %s, stderr = %s", code, stdout.String(), stderr.String())
	}
	...
	stdout.Reset()
	stderr.Reset()
	code = app.run([]string{"dev", "seed-scene", "--json", "--region", "dev", "--house-id", "200171", "--name", "Codex Dev Test Scene", "--allow-write-dev"}, strings.NewReader(""), &stdout, &stderr)
	if code != exitInvalidInput {
		t.Fatalf("exit code = %d, stdout = %s, stderr = %s", code, stdout.String(), stderr.String())
	}
...
func TestDevSeedSceneCreatesSceneUsingStoredCredentials(t *testing.T) {
	var stdout bytes.Buffer
	var stderr bytes.Buffer
	...
	server := httptest.NewServer(http.HandlerFunc(func(writer http.ResponseWriter, request *http.Request) {
		...
		case "/apis/iot/v2/thing/manage/house/200171/scene/r/info/1/100":
			...
			_, _ = writer.Write([]byte(`{"success":true,"data":{"rows":[{"id":"scene-created","name":"Codex Dev Test Scene"}]}}`))
	...
	code := app.run([]string{"dev", "seed-scene", "--json", "--region", "dev", "--name", "Codex Dev Test Scene", "--device-id", "50018330", "--device-name", "light-dali开关灯-17000002-01", "--allow-write-dev"}, strings.NewReader(""), &stdout, &stderr)
	if code != exitOK {
	...
	if createBody["houseId"] != float64(200171) {
	...
	if response["sceneId"] != "scene-created" || response["created"] != true || response["verifiedBy"] != "scene_list" {
