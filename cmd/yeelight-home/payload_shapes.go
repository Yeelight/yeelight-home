...
func normalizedLightingDesignTopologyShape() map[string]any {
	return map[string]any{
		"gateways": []any{
			map[string]any{
				"localId":   "required local gateway id referenced by rooms/devices",
				"localName": "required when creating a new gateway slot",
				"id":        "optional existing cloud gateway slot id for update/mapping",
				"pid":       "required gateway product id; Runtime defaults for natural topology",
				"pcId":      "required gateway product category id; Runtime defaults for natural topology",
				"mac":       "optional",
				"role":      "optional 0 or 1",
			},
		},
		"rooms": []any{
			map[string]any{
				"localId":     "required local room id referenced by devices/groups",
				"localName":   "required when creating a new room",
				"gatewayIds":  "optional local gateway id list",
				"id":          "optional existing cloud room id",
				"areaId":      "optional local area id",
				"cloudAreaId": "optional cloud area id",
			},
		},
		"devices": []any{
			map[string]any{
				"localId":              "required local device slot id",
				"localName":            "required slot/device display name",
				"gatewayDeviceId":      "required local gateway id unless cloudGatewayDeviceId is provided",
				"cloudGatewayDeviceId": "optional cloud gateway slot id",
				"roomId":               "required local room id unless cloudRoomId is provided",
				"cloudRoomId":          "optional cloud room id",
				"addr":                 "required non-negative local address",
				"pid":                  "required product id",
				"pcId":                 "optional product category id",
				"mac":                  "optional",
				"connectType":          "optional -1..6; Runtime may infer from product evidence",
				"attrs":                "optional metadata object such as designSlot, materialCode, productName, notes",
				"operation":            "optional source-backed design sync operation add/modify/remove/bind/unbind only when Runtime accepts it",
			},
		},
		"deviceGroups": []any{
			map[string]any{
				"localId":   "required local group id",
				"localName": "required when creating a new group",
				"id":        "optional existing cloud group id",
				"roomId":    "optional local room id",
				"deviceIds": "local device ids included in the group",
				"operation": "optional source-backed design sync operation add/modify/remove only when Runtime accepts it",
			},
		},
		"operationVocabulary": map[string]any{
			"nodes":  "add creates a design slot, modify updates a slot, remove deletes a slot, bind/unbind represent onboarding relationship metadata only when Runtime explicitly supports them",
			"groups": "add/modify/remove; nested group node rows may use add/remove membership operations",
			"rooms":  "add/modify/remove; nested room node rows may use add/remove membership operations",
			"scenes": "add/modify/remove; scene node rows may use add/remove only when executable targets are known",
			"rules":  "add/modify/remove; params must follow Runtime automation condition/action shape",
			"areas":  "add/modify/remove; nested area node rows may use add/remove membership operations",
		},
	}
}
