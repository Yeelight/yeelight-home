...
func TestInvokeEntityRenameBatchDryRunPreviewsWithoutWriting(t *testing.T) {
	var gotCalls []string
...
func TestInvokeEntityRenameBatchExecutesDirectly(t *testing.T) {
	...
	t.Setenv("YEELIGHT_API_BASE_URL", server.URL+"/apis/iot")
	app := newInvokeTestApp(t, "Bearer token-rename-batch-secret", "client-rename-batch-1", "200171")
	input := `{"contractVersion":"1.0","requestId":"req-rename-batch-execute","locale":"zh-CN","utterance":"把主灯改名为阅读主灯，把已有情景改名为睡前晚安","intent":"entity.rename.batch","parameters":{"houseId":"200171","items":[{"entityType":"device","id":"50018330","newName":"阅读主灯"},{"entityType":"scene","currentName":"已有情景","newName":"睡前晚安"}]}}`
	var stdout bytes.Buffer
	var stderr bytes.Buffer
	...
	response := decodeInvokeResponse(t, stdout.Bytes())
	if response["status"] != "success" || response["traceId"] != "entity-batch-rename-execute" {
		t.Fatalf("response = %#v", response)
	}
	result := response["result"].(map[string]any)
