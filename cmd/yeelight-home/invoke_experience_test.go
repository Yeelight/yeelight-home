...
func TestInvokeLightingExperienceApplyUsesReviewedLightWrapper(t *testing.T) {
	...
	input := `{"contractVersion":"1.0","requestId":"req-exp-apply","locale":"zh-CN","utterance":"给主灯应用观影体验","intent":"lighting.experience.apply","targets":[{"entityType":"device","id":"device-1"}],"parameters":{"mood":"观影"}}`
	var stdout bytes.Buffer
	var stderr bytes.Buffer
	code := app.run([]string{"invoke", "--stdin"}, strings.NewReader(input), &stdout, &stderr)
	if code != exitOK {
...
func TestInvokeLightingExperienceApplyRequiresDeviceTarget(t *testing.T) {
	...
	input := `{"contractVersion":"1.0","requestId":"req-exp-room","locale":"zh-CN","utterance":"给客厅应用观影体验","intent":"lighting.experience.apply","targets":[{"entityType":"room","id":"room-1"}],"parameters":{"mood":"观影"}}`
	var stdout bytes.Buffer
	var stderr bytes.Buffer
	code := app.run([]string{"invoke", "--stdin"}, strings.NewReader(input), &stdout, &stderr)
	if code != exitOK {
