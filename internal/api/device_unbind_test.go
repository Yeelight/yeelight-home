...
func TestDeviceUnbindClientUsesOptionsAndVerifiesRemoval(t *testing.T) {
	...
	if !result.Verified || result.VerifiedBy != "entity.list:missing" || result.DeviceID != "device-1" {
		t.Fatalf("result = %#v", result)
	}
}

func TestDeviceUnbindClientVerifiesUnboundSlot(t *testing.T) {
	deviceBind := true
	server := httptest.NewServer(http.HandlerFunc(func(writer http.ResponseWriter, request *http.Request) {
		writer.Header().Set("Content-Type", "application/json")
		switch request.URL.Path {
		case "/apis/iot/v2/thing/manage/house/200171/device/r/info/1/100":
			if deviceBind {
				_, _ = writer.Write([]byte(`{"success":true,"data":{"rows":[{"id":"device-1","name":"虚拟灯","bind":true}]}}`))
				return
			}
			_, _ = writer.Write([]byte(`{"success":true,"data":{"rows":[{"id":"device-1","name":"虚拟灯","bind":false}]}}`))
		case "/apis/iot/v2/thing/manage/house/200171/area/r/info/1/100",
			"/apis/iot/v2/thing/manage/house/200171/room/r/info/1/100",
			"/apis/iot/v2/thing/manage/house/200171/group/r/info/1/100",
			"/apis/iot/v2/thing/manage/house/200171/scene/r/info/1/100",
	...
	if !result.Verified || result.VerifiedBy != "entity.list:bind=false" || result.DeviceID != "device-1" {
		t.Fatalf("result = %#v", result)
	}
}
